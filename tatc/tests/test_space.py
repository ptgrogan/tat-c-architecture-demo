...
class TestSatellite(unittest.TestCase):
    def test_from_json_basic(self):
        o = Satellite.from_json('{"name": "Landsat 8", "mass": 2750, "volume": 43.2, "power": 1550, "commBand": ["X"]}')
    ...
    def test_to_json_basic(self):
        d = json.loads(Satellite(name="Landsat 8", mass=2750, volume=43.2, power=1550, commBand=["X"]).to_json())
    ...
    def test_from_json_architecture(self):
        o = Satellite.from_json('{"orbit": {"@type": "Orbit"}, "payload": {"@type": "Instrument"}}')
        self.assertIsInstance(o.orbit, Orbit)
        self.assertIsInstance(o.payload, Instrument)
    def test_from_json_architecture_list(self):
        o = Satellite.from_json('{"orbit": {"@type": "Orbit"}, "payload": [{"@type": "Instrument"}, {"@type": "Instrument"}]}')
        self.assertIsInstance(o.orbit, Orbit)
    ...
    def test_to_json_architecture(self):
        ...
        self.assertEqual(d.get("payload").get("@type"), "Instrument")
    def test_to_json_architecture_list(self):
        ...
        self.assertEqual(d.get("payload")[0].get("@type"), "Instrument")
        self.assertEqual(d.get("payload")[1].get("@type"), "Instrument")
...
class TestConstellationDeltaHomogeneous(unittest.TestCase):
    def test_from_json_basic(self):
        o = Constellation.from_json('{"constellationType": "DELTA_HOMOGENOUS", "numberSatellites": 2, "numberPlanes": 2, "orbit": {"orbitType": "circular", "altitude": 405, "inclination": 51.64}}')
        self.assertEqual(o.constellationType, ConstellationType.DELTA_HOMOGENOUS)
    ...
    def test_from_json_range(self):
        o = Constellation.from_json('{"constellationType": "DELTA_HOMOGENOUS", "numberSatellites": {"minValue": 1, "maxValue": 2}, "numberPlanes": {"minValue": 1, "maxValue": 2}, "orbit": {"altitude": 405, "inclination": 51.64}}')
    ...
    def test_to_json(self):
        d = json.loads(Constellation(constellationType="DELTA_HOMOGENOUS", numberSatellites=2, numberPlanes=2, orbit=Orbit(orbitType="circular", altitude=405, inclination=51.64)).to_json())
        self.assertEqual(d.get("constellationType"), "DELTA_HOMOGENOUS")
